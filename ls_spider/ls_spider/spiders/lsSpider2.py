...
class Lsspider1Spider(scrapy.Spider):
    ...
    custom_settings = {
                    'LOG_LEVEL': 'DEBUG',
                    'LOG_FILE': 'D:\\pyrun\\log\\level_2_log_%s.log' % time.strftime('%Y%m%d_%H%M%S',time.localtime()),
    ...
    def start_requests(self):
        # 删除result2目录下的所有文件 # spider0 已经删除
        # b = os.path.join(os.getcwd(),"result2")
        # f= os.listdir(b)
        # for i in f:
        #     os.remove(os.path.join(b,i))

        # 从redis中读出百度搜索到的网页链接，然后进行内容的读取
        self.start_urls=list(self.r.smembers('urllist2'))
        ...
        for i, a in enumerate(response.xpath('//a')):
            ...
            if url != None and url != '':
                if url.find("javascript:") < 0 and url.find("baidu") < 0 and url.find("http:") > -1 and url.find("zhihu")<0 and url.find('360doc')<0:
                    ...
                    self.r.sadd('urllist3',url)
            self.r.set('num_3', self.r.scard('urllist3'))
        yield item
def randam_letter():
